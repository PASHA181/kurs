<div class="mainCont">
<form action="https://money.yandex.ru/quickpay/confirm.xml" target="_top" method="POST">
<input type="hidden" value="<?echo K::get('settings')->yandexPurse?>" name="receiver">
<input type="hidden" value="Оплата заказа № <?=$order?> " name="label">
...
<input type="hidden" value="Оплата заказа № <?=$order?>" name="targets">
<input type="hidden" value="<?=$summ?>" name="sum">
<input type="submit" class="btn" value="Оплатить" name="submit-button">

</form>
<p>
 <br/>
 <em>
 Вы сможете сделать оплату позже, перейдя по ссылке из высланного вам письма.
 <br/>
 Или перечислить деньги на наш <b><span style="color:red" >Я</span>ндекс</b> кошелек <b><?=$_SESSION['settings']['yandex-purse']?></b> указав номер заказа.
 <br/>
 А также вы сможете оплатить свой заказ спомощью нашей <a href="#">формы оплаты заказов</a>
 </em>
 </p>
<br/>
